...
Here is the uncomfortable truth: the cloud provider's API was always the real source of truth. Your IaC is a projection of objective. When `terraform plan` shows changes, it does not mean your code is right. It means reality diverged.
...
**Sub-problem A: Capturing the objective**

The objective is the goal behind a change. Not the specific value, but what it serves.

HPA sets replicas=10. The objective is availability. Cost optimizer sets replicas=3. The objective is cost efficiency. Security scanner patches an image. The objective is vulnerability remediation.

Objectives are relatively stable categories: availability, cost, security, compliance, performance. Most infrastructure changes map to one of these.

But the objective alone is not enough. Two actors can share the same objective and still conflict. Two different availability concerns. Two different compliance requirements. You need more context.

**Sub-problem B: Capturing the motivation**

Motivation is the *why now*. The context that explains urgency and priority.

The on-call engineer wants replicas=10. Objective: availability. Motivation: incident response, primary is throwing 500s, customers are affected.

The platform team also wants replicas=10. Objective: availability. Motivation: scheduled capacity for tomorrow's launch.

Same objective. Same desired state. Very different motivations. If you had to roll one back, you would pick the scheduled change over the active incident response.

Today, we capture almost none of this. Audit logs show *what* changed and *who* (which service account). Git captures prose for humans. Nothing is machine-readable. Nothing connects the change to its objective or motivation.

**Sub-problem C: Deciding who wins**

Even if we capture the objective and motivation, we still need logic to arbitrate.

- **HPA vs cost optimizer:** Both valid. Objective: availability vs cost. But if HPA's motivation is "active traffic spike" and cost optimizer's motivation is "monthly budget target," the spike wins.
- **Security patch vs app stability:** Objective: security vs availability. Motivation: CVE with known exploit vs potential compatibility risk. Probably security wins—but what if the app is revenue-critical during a peak sales period?
- **Hotfix vs declared state:** On-call engineer's objective: availability. Motivation: incident response. Terraform's objective: consistency. Motivation: it is 9am and this is what the cron job does. The hotfix should win—but nothing in our tooling knows that.

Look at what we have today:

- **Kubernetes SSA:** Tracks *who* touched each field, not *why*. Conflict resolution is "force" or "fail."
- **OPA/Gatekeeper:** Binary allow/deny. Cannot arbitrate between two valid objectives.
...
None of these capture the objective. None of them capture motivation. None of them have arbitration logic that weighs context.
...
The one domain that has thought carefully about goal-declaration is [Intent-Based Networking](https://datatracker.ietf.org/doc/rfc9315/). They call it *intent*; it is what I am calling *objective*.
...
It does not solve the *why-should-this-win* problem. When two objectives conflict, RFC 9315 says the system should "help users to properly choose between intent alternatives."

In other words: escalate to a human. The arbitration logic itself is out of scope.

And even IBN's conflict model assumes objectives come from *humans* who can be consulted. It does not address autonomous actors making changes at machine speed with no human in the loop.

Infrastructure has even less. Our tools do not capture the objective or motivation. They do not have arbitration logic. They assume single-writer.
...
2. **Competing motivations**—the semantic problem of capturing the *objective* (what goal a change serves), the *motivation* (why now, what context), and then deciding whose motivation wins when they conflict. This is largely unsolved for infrastructure.

The tools we have—Terraform, Pulumi, ArgoCD, SSA—were built for a world of human operators making deliberate changes through controlled processes. They do not capture the objective. They do not capture motivation. They do not arbitrate.
...
**Update, 2026-05-22:** I have stopped using *intent* for the goal a change serves and switched to *objective*. The shape of the argument has not changed, but the word has. *Intent* gets used too loosely—"the user's intent," "what the agent intended," "intent-based X." *Objective* is more precise: it names the goal a change is serving (availability, cost, security), separately from the motivation (why now, what context). The rest of the post reflects the new vocabulary; the IBN section still says *intent* because that is what RFC 9315 calls it.
