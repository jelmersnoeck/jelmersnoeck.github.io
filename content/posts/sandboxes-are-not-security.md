...
Different, not independent. Authorisation binds to an identity, and identity on a host is only as trustworthy as the isolation that stops the next process over from claiming it. Isolation is the floor authorisation stands on. The industry's mistake was not building the floor. It was calling the floor the house.
...
A sandbox can lock down egress and refuse to let the agent talk to anything except an allow-listed set of endpoints. That helps, until the endpoint is one the agent is allowed to reach and the compromise becomes a credential leak instead of an exfil. The right answer to that shape of problem is a gateway sitting between the agent and the resource, brokering the call and watching what the credential is used for; another post. The sandbox has one indispensable job in that story: making the gateway the only road. Egress control is not the security decision. It is what makes the security decision unavoidable.
...
Some protocols will fight this. Postgres authenticates a connection, not a query; once the session is up, every statement rides the grant that opened it. Per-turn binding there means gating in the path, statement by statement, or shrinking the injected credential to the blast radius of a single task. The bar doesn't move. The protocol decides how expensive meeting it gets.
...
What they should stop selling is the idea that this is the security story. Sold alone, the sandbox is a hygiene primitive with a security pricetag. Wired to a workload identity and a policy authority, it is something else entirely: the attestation root, the enforcement point, the thing that makes the gateway unbypassable. That second product is worth the pricetag. Nobody in the category is shipping it. Calling the part the whole has held the industry up for two years and it is starting to cost real companies real money.

The sandbox is not security. The sandbox is where security executes, once someone builds the part that decides.
